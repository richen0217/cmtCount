...
namespace WinComment
{
    ...
    {
        private static EnAndDe ende = new EnAndDe();
        #region 私有构造函数和方法
        ...
        {
            string database = ConfigurationManager.AppSettings["dbConfig"];
            string serCom = ende.Decrypt(ConfigurationManager.AppSettings["serCom"]);
            string unCom = ende.Decrypt(ConfigurationManager.AppSettings["unCom"]);
            string pwdCom = ende.Decrypt(ConfigurationManager.AppSettings["PwdCom"]);

            string ComStr = string.Format(@"server={0};database={1};user id={2};password={3};Connect Timeout=3;", serCom, database, unCom, pwdCom);
            return ComStr;
        }
        /// <summary> 
        /// 一个有效的数据库连接对象 
